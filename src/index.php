...
<head>
    ...
    <style>
        input:disabled { background-color: #1e293b; color: #94a3b8; cursor: not-allowed; }
    </style>
</head>
<body class="bg-slate-950 text-slate-200 min-h-screen p-4 md:p-10">
    <div class="max-w-5xl mx-auto">
        ...
        <div id="section-ipv4" class="space-y-6">
            <div class="bg-slate-900 p-6 rounded-xl border border-slate-800 shadow-2xl">
                <div class="grid md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-500 mb-2">Network Address (Randomized Only)</label>
                        <div class="flex gap-2">
                            <input type="text" id="ipv4_addr" disabled class="w-full p-3 rounded bg-slate-800 border border-slate-700 font-mono">
                            <button onclick="randomizeIP()" class="bg-blue-600 hover:bg-blue-500 px-4 rounded">🎲</button>
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-500 mb-2">Prefix (/1 to /30)</label>
                        <input type="number" id="ipv4_prefix" value="24" class="w-full p-3 rounded bg-slate-800 border border-slate-700 font-mono focus:border-blue-500 outline-none">
                ...
                <div class="mb-6">
                    ...
                    <div id="host-rows" class="space-y-2 mb-4">
                        <div class="flex gap-2 host-row">
                            <input type="text" placeholder="Network Name (e.g. Sales)" class="h-name w-1/2 p-2 rounded bg-slate-800 border border-slate-700">
                            <input type="number" placeholder="Hosts" class="h-count w-1/4 p-2 rounded bg-slate-800 border border-slate-700">
                ...
                <button onclick="calculateIPv4()" class="w-full bg-blue-600 hover:bg-blue-500 p-4 rounded-lg font-bold text-lg shadow-lg transition">Generate Subnet Table</button>
            </div>
        ...
        <!-- IPv6 SECTION (As per your reference image) -->
        <div id="section-ipv6" class="hidden space-y-6">
            <div class="bg-slate-900 p-8 rounded-2xl border border-slate-800 shadow-2xl">
                ...
                <div class="space-y-4">
                    ...
                    <input type="range" id="sla-slider" min="0" max="65535" value="0" class="w-full h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-blue-600" oninput="updateIPv6(this.value)">
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function switchMode(mode) {
            ...
            document.getElementById('btn-ipv4').className = mode === 'ipv4' ? 'px-6 py-2 rounded-md bg-blue-600 text-white font-bold' : 'px-6 py-2 rounded-md text-slate-400';
            document.getElementById('btn-ipv6').className = mode === 'ipv6' ? 'px-6 py-2 rounded-md bg-blue-600 text-white font-bold' : 'px-6 py-2 rounded-md text-slate-400';
        }
        ...
        function addHostRow() {
            ...
            div.innerHTML = `<input type="text" placeholder="Network Name" class="h-name w-1/2 p-2 rounded bg-slate-800 border border-slate-700">
                             <input type="number" placeholder="Hosts" class="h-count w-1/4 p-2 rounded bg-slate-800 border border-slate-700">
                             <button onclick="this.parentElement.remove()" class="text-red-500 px-2">✕</button>`;
        ...
        async function calculateIPv4() {
            ...
            if (prefix === 0 || prefix > 32) {
                errEl.innerText = "Input Error: Prefix must be between 1 and 30.";
                errEl.classList.remove('hidden'); return;
            } else if (prefix === 31) {
                errEl.innerText = "Notice: /31 is used for point-to-point links (RFC 3021).";
                errEl.classList.remove('hidden'); return;
            } else if (prefix === 32) {
                errEl.innerText = "Notice: /32 is frequently used for loopback interfaces.";
                errEl.classList.remove('hidden'); return;
            }

            const hosts = Array.from(document.querySelectorAll('.host-row')).map(row => ({
                name: row.querySelector('.h-name').value || "Unnamed",
            ...
            const res = await fetch('api.php?action=calculate', {
                ...
                body: JSON.stringify({ base_ip: document.getElementById('ipv4_addr').value, prefix, hosts })
            });
            ...
            if (data.status === 'success') {
                ...
                document.getElementById('ipv4-tbody').innerHTML = data.subnets.map(s => `
                    <tr class="border-t border-slate-800">
                        ...
                        <td class="p-4 font-mono">${s.broadcast}</td>
                        <td class="p-4 text-xs text-slate-500 font-mono">${s.subnet_mask}</td>
                    </tr>
                `).join('');
                document.getElementById('steps-list').innerHTML = data.steps.map(s => `<li>${s}</li>`).join('');
            } else {
                alert(`Hindi Kasya! Based on your requirements, use a /${data.suggestion} prefix.`);
            }
        }

        // --- IPv6 Logic ---
        function updateIPv6(val) {
            const hex = parseInt(val).toString(16).padStart(4, '0');
            document.getElementById('ipv6-sla-display').innerText = hex.toUpperCase();
